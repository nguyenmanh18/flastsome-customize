    ...
    public function ywar_product_reviews_submit_reviews_denied_text( $message, $product_id ) {
        if ( $this->is_reviews_limited() && $this->customer_reviews_for_product( $product_id ) ) {
            ...
            if ( $this->allow_edit_review )
                returnesc_html__( "You can't add more than one review for this product. Would you like to edit the review you have already written?", 'yith-woocommerce-advanced-reviews' );
            else
                returnesc_html__( "You can't add more than one review for this product.", 'yith-woocommerce-advanced-reviews' );
        }
    ...
    public function verify_comment_captcha($commentdata)
    {
        ...
        if (isset($_POST['yith_ywar_recaptcha_product']) && $commentdata['comment_type'] == 'review') {
            ...
            if (isset($_POST['g-recaptcha-response'])) {
                ...
                echoesc_html__("Bots are not allowed to submit comments. If you are not a bot then please enable JavaScript in browser.");
                return null;
            }
        }
        return $commentdata;
